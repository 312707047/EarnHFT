...
def get_best_model(label_name, root_path=root_path):
    ...
    with Pool() as p:
        result_dicts = p.map(process_coffient, coffient_list)
    ...
    for i in range(5):
        ...
        model_path_list.append(
            os.path.join(
                ...
                para,
                "seed_12345",
                "epoch_{}".format(epoch_number),
                "trained_model.pkl",
            )
        )
    model_path_list = list(set(model_path_list))
    model_list = [torch.load(model_path) for model_path in model_path_list]
